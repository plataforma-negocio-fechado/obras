...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-brand-cream text-brand-navy">
    ...
    <main className={`${collapsed && !isMobile ? "pl-[76px]" : "pl-[252px]"} transition-all duration-200 ${isMobile ? "!pl-0" : ""}`}>{isMobile && <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-brand-cream bg-brand-cream/95 px-4 backdrop-blur"><button onClick={() => setMobileOpen(true)} className="grid h-9 w-9 place-items-center rounded-md bg-brand-navy text-white" aria-label="Abrir menu"><PanelLeft className="h-4 w-4"/></button><span className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-brand-navy">Negócio Fechado · Obras</span><X className="h-4 w-4 opacity-0"/></header>}{children}</main>
  </div>;
}
