import { BriefcaseBusiness, MapPin, Phone, Share2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import type { ProfessionalProfile } from "@/data/professionalProfiles";

export default function ProfessionalProfilePage({ profile }: { profile: ProfessionalProfile }) {
  const publicUrl = `${window.location.origin}${window.location.pathname}#/perfil/${profile.slug}`;
  const share = async () => {
    if (navigator.share) return navigator.share({ title: `${profile.name} — Perfil Negócio Fechado`, url: publicUrl });
    await navigator.clipboard?.writeText(publicUrl);
    window.alert("Link copiado.");
  };

  return <main className="min-h-screen bg-[#f8f5ed] text-[#102e46]">
    <div className="bg-[#102e46] px-5 py-2 text-center font-mono text-[10px] uppercase tracking-[.16em] text-[#dce5eb]">Negócio Fechado · Perfil profissional</div>
    ...
    <section className="border-b border-[#d8d2c5] bg-[#fffdf8]">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-12 md:grid-cols-[1.2fr_.8fr] md:px-8 md:py-20">
        <div>
          <div className="mb-6 flex items-center gap-4"><div className="flex h-20 w-20 items-center justify-center rounded-full bg-[#102e46] font-display text-3xl font-semibold text-white">{profile.initials}</div><p className="font-mono text-[10px] font-bold uppercase tracking-[.17em] text-[#d96b32]">{profile.registration}</p></div>
