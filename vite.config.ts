...
function trimLogFile(logPath: string, maxSize: number) {
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      keptLines.unshift(lines[i]); keptBytes += lineBytes;
    }
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/__manus__/logs", (req, res, next) => {
        ...
        if (reqBody && typeof reqBody === "object") { try { handlePayload(reqBody); } catch (e) { res.writeHead(400); res.end(JSON.stringify({ success: false, error: String(e) })); } return; }
        let body = "";
